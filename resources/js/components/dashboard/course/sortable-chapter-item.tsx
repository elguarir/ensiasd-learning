...
import { GripVertical, MoreHorizontal, Edit, Trash2, Plus } from "lucide-react";
    ...
    return (
      <>
        <div ref={setNodeRef} style={style}>
          <AccordionItem
            value={valueKey}
            className="bg-card transition-all duration-300 overflow-hidden"
          >
            <AccordionTrigger className="px-4 py-3 hover:no-underline">
              <div className="flex w-full items-center gap-4">
                ...
                <div className="min-w-0 flex-1 text-left">
                  <h3 className="truncate text-lg font-semibold">{chapter.title}</h3>
                  <Badge variant="outline" className="mt-1">
                    {chapter.resources?.length || 0}{" "}
                    {(chapter.resources?.length || 0) === 1 ? "resource" : "resources"}
                  </Badge>
                </div>
